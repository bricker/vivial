...
async def test_map_link_content() -> None:
    ...
    mockito.when2(GitHubClient.get_file_content, **mockito.kwargs) \
        .thenReturn("dummy gh content") \
        .thenReturn("dummy gh content")
...
async def test_subscribe_successful_subscription() -> None:
    mockito.when2(GitHubClient.get_repo, **mockito.kwargs) \
        .thenReturn(GithubRepository(node_id="1", full_name="eave-fyi/eave-monorepo")) \
        .thenReturn(GithubRepository(node_id="1", full_name="the-org/repo-name"))
    mockito.when2(eave_core.create_subscription, **mockito.kwargs).thenReturn(None)
