...
class UtmCookiesTestBase(UtilityBaseTestCase):
    async def asyncSetUp(self) -> None:
        ...
        self.mock_scope["query_string"] = f"utm_campaign={self.data_campaign}&UTM_TERM={self.data_term}&gclid={self.data_gclid}&ignored_param={self.data_ignored_param}"
...
class UtmCookiesTest(UtmCookiesTestBase):
    ...
    async def test_set_cookies_visitor_id_not_set(self):
        set_tracking_cookies(cookies=self.mock_request.cookies, query_params=self.mock_request.query_params, response=self.mock_response)
