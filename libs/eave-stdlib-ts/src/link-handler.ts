...
export function filterSupportedLinks(
  urls: Array<string>,
): Array<Pair<string, LinkType>> {
 ...
 */
export async function mapUrlContent({
  origin,
  teamId,
  urls,
  ctx,
}: RequestArgsTeamId & { urls: Array<Pair<string, LinkType>> }): Promise<
  Array<string | null>
> {
...
> {
  ...
  urls.forEach(async (linkContext) => {
    ...
    const maybeSubscription = await createSubscription({
      origin,
      teamId,
      url,
      linkType,
  ...
  switch (linkType) {
    case LinkType.github: {
      const subscriptionResponse =
        await CreateGithubResourceSubscriptionOperation.perform({
          ctx,
          origin,
          teamId,
          input: {
