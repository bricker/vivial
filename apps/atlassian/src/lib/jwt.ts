...
//     return Promise.reject(new Error('JWT Verification Failed, wrong qsh'));
//   }
// };
...
//     return Promise.reject(new Error(`Unable to get public key for keyId ${keyId}`));
//   }
...
//  */
// export const verifySymmetricJWTToken = async (request: Request, token?: string): Promise<void> => {
...
//   if (!jiraTenant) {
//     return Promise.reject(new Error("Jira Tenant doesn't exist"));
//   }
...
//     return Promise.reject(new Error(`JWT verification failed: ${e}`));
//   }
// };
...
//     return Promise.reject(new Error('JWT claim did not contain the issuer (iss) claim'));
//   }

//   // Make sure the AUD claim has the correct URL
//   if (!unverifiedClaims?.aud?.[0]?.includes(envVars.APP_URL)) {
//     return Promise.reject(new Error('JWT claim did not contain the correct audience (aud) claim'));
//   }
...
//     return Promise.reject(new Error('JWT validation Failed, no qsh'));
//   }
...
//     return Promise.reject(new Error('JWT validation failed, token is expired'));
//   }
