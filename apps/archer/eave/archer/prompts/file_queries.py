...
class FileQueryResponse:
    ...
    third_party_references: list[str]

async def query_file_contents(filepath: str, model: _o.OpenAIModel, github_ctx: GithubContext) -> FileQueryResponse | None:
    print("\n\n")
    ...
    prefix_messages = [
        _o.ChatMessage(role=_o.ChatRole.SYSTEM, content=_o.formatprompt(
            ...
            1. Is this code setting up an HTTP server and API endpoints? If so, create a simple, human-readable name for this API, and a short description of the purpose of the API.
            2. Does this code reference or make calls to any other APIs? If so, create a human-readable name for each one, and list them.

            It is very important that the list of API references only includes APIs that are likely to be external to this codebase and accessed over HTTP.

            Examples of valid APIs:
            - OpenAI, Slack, SendGrid, Stripe, etc.
            - Google Cloud, AWS, Azure, etc.
            - Postgres, MySQL, Spanner, etc.
            ...
            - third_party_references (String[]): A list of the names you created for any third-party references in this code. If there are none, this should be an empty array.
        ...
        _o.ChatMessage(role=_o.ChatRole.ASSISTANT, content=_o.formatprompt(
            """
            {"service_name": "Eave Core API", "service_description": "The Core API service for Eave, providing user information.", "third_party_references": ["OpenAI", "Confluence"]}
            """
        )),
    ]
    ...
    PROMPT_STORE["file_queries"] = (params, response)
