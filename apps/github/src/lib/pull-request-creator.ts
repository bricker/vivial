...
export class PullRequestCreator {
  ...
  /**
   * Opens a new pull request against event.pull_request.base.ref (same base as PR that triggered this event) on the specified branch.
   ...
   * The `branchNodeId` is used as the `refNodeId` parameter for the `deleteRef` mutation.
   * 
   * More details can be found at: https://docs.github.com/en/graphql/reference/mutations#deleteref
   */
  ...
  /**
   * Asynchronously creates a pull request with the provided parameters. If the branch already exists, it pushes a commit to it. 
   * If not, it creates a new branch and pushes a commit. If an open pull request already exists for the branch, it appends new commits to it. 
   * If not, it opens a new pull request. If the attempt to open a pull request fails, it deletes the newly created branch to prevent orphaning.
   * 
   ...
   * 
   * @throws Will throw an error if the attempt to open a pull request fails.
   */
 ...
 * @throws {TypeError} If `a` or `b` is null.
 */
