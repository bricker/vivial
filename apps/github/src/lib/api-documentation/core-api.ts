...
export class CoreAPIData {
  ...
  static async load({ ctx, teamId, eaveGithubRepoInput }: CtxArg & { teamId: string; eaveGithubRepoInput: GithubRepoInput }) {
    let team: Team;
...

    {
      ...
      assert(r, `No repo found in Eave for ID ${eaveGithubRepoInput.external_repo_id}`);
      eaveGithubRepo = r;
    }

    {
      ...
      response.documents.forEach((doc) => {
        if (doc.file_path) {
          const key = githubDocumentKey({ filePath: doc.file_path, externalRepoId: eaveGithubRepo.external_repo_id });
          existingGithubDocuments[key] = doc;
        }
      });
    }

    return new CoreAPIData({ team, ctx, eaveGithubRepo, existingGithubDocuments })
  }

  private constructor({ team, ctx, eaveGithubRepo, existingGithubDocuments }: CtxArg & EaveGithubRepoArg & { team: Team, existingGithubDocuments: GithubDocumentTable }) {
    this.ctx = ctx
    this.team = team
  ...
  async createPlaceholderGithubDocument({
    apiName, documentationFilePath,
  }: { apiName: string, documentationFilePath: string }): Promise<GithubDocument> {
  ...
  setGithubDocument({ document }: { document: GithubDocument }) {
    if (document.file_path !== null) {
      ...
      const key = githubDocumentKey({ filePath: document.file_path, externalRepoId: this.eaveGithubRepo.external_repo_id });
      this.eaveGithubDocuments[key] = document
    }
  }

  getGithubDocument({ filePath }: { filePath: string }): GithubDocument | undefined {
    const key = githubDocumentKey({ filePath, externalRepoId: this.eaveGithubRepo.external_repo_id })
    return this.eaveGithubDocuments[key];
  }
}

function githubDocumentKey({ externalRepoId, filePath }: { externalRepoId: string, filePath: string }): string {
