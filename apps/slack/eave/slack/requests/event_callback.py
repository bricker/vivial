...
class SlackEventCallbackHandler(HTTPEndpoint):
    ...
    async def post(self, request: Request) -> Response:
        ...
        eaveLogger.info("Request: POST /slack/events", extra=extra)
        ...
        if not self._is_signature_valid():
            eaveLogger.warning("Invalid Slack signature", extra=extra)
        ...
        if not await self._is_watched_event():
            eaveLogger.debug("Ignoring unmatched Slack event", extra=extra)
            return default_success_response
