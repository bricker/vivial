...
import {
  ...
  ADFRootNode,
  ADFTextNode,
} from "@eave-fyi/eave-stdlib-ts/src/connect/types/adf.js";
...
import {
  ...
  SearchDocumentsResponseBody,
} from "@eave-fyi/eave-stdlib-ts/src/core-api/operations/documents.js";
import {
  LogContext,
  eaveLogger,
...
export default async function commentCreatedEventHandler({
  req,
  res,
  jiraClient,
}: {
  ...
  const mentionAccountIds = Array.from(
    payload.comment.body.matchAll(ACCOUNT_ID_RE),
  );
  ...
  if (!team) {
    eaveLogger.warning(
      "No teamId available",
      { clientKey: jiraClient.client.clientKey },
      ctx,
    );
    res.sendStatus(400);
    return;
...
function buildEaveResponse({
  searchResults,
  payload,
}: {
  searchResults: SearchDocumentsResponseBody;
  payload: JiraCommentCreatedEventPayload;
}): ADFRootNode {
  let content: ADFNode[];
  ...
  if (searchResults.documents.length > 0) {
    const listItemNodes = searchResults.documents.map(
      (document): ADFListItemNode => {
