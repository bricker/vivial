...
export function InternalApiRouter(_args: { addon: AddOn }): Router {
  const router = Router();
  router.use(rawJsonBody, requireHeaders(EAVE_SIGNATURE_HEADER, EAVE_TEAM_ID_HEADER, EAVE_ORIGIN_HEADER), originMiddleware, signatureVerification({ audience: EaveApp.eave_jira_app }));
