...
class TestCostBreakdown(BaseTestCase):
    async def test_cost_breakdown_multiplication(self) -> None:
        cost_breakdown = CostBreakdown(max_base_cost_cents=100, fee_cents=10, tax_cents=5)

        assert cost_breakdown.calculate_total_cost_cents() == 115

        doubled_breakdown = cost_breakdown * 2
        assert doubled_breakdown.max_base_cost_cents == 200
    ...
    async def test_cost_breakdown_addition(self) -> None:
        cost_breakdown1 = CostBreakdown(max_base_cost_cents=100, fee_cents=10, tax_cents=5)
        cost_breakdown2 = CostBreakdown(max_base_cost_cents=200, fee_cents=20, tax_cents=10)

        added_breakdown = cost_breakdown1 + cost_breakdown2
        assert added_breakdown.max_base_cost_cents == 300
    ...
    async def test_cost_breakdown_add_equals(self) -> None:
        ...
        assert cost_breakdown1.max_base_cost_cents == 300
