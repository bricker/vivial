...
class TestEventbriteEventOrm(BaseTestCase):
    async def test_new_record(self) -> None:
        async with self.db_session.begin() as session:
            ...
            obj.update(
                ...
                coordinates=GeoCoordinates(lat=self.anylatitude("lat"), long=self.anylongitude("long"))
            )
    ...
    async def test_update_existing_record(self) -> None:
        ...
        async with self.db_session.begin() as session:
            ...
            newcoords = GeoCoordinates(lat=self.anylatitude("new lat"), long=self.anylongitude("new long"))
