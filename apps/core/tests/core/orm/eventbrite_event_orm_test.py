...
class TestEventbriteEventOrm(BaseTestCase):
    async def test_new_event_record(self) -> None:
        async with self.db_session.begin() as session:
            ...
            obj.update(
                title=self.anystr(),
                vivial_category_id=self.anyuuid(),
                vivial_format_id=self.anyuuid(),
    ...
    async def test_update_existing_record(self) -> None:
        ...
        async with self.db_session.begin() as session:
            ...
            qobj.update(
                ...
                vivial_category_id=self.anyuuid("new vivial_category_id"),
                vivial_format_id=self.anyuuid("new vivial_format_id"),
        ...
        async with self.db_session.begin() as session:
            ...
            assert qobj.vivial_category_id == self.getdatetime("new vivial_category_id")
            assert qobj.vivial_format_id == self.getdatetime("new vivial_format_id")
