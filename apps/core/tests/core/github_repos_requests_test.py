...
class TestGithubRepoRequests(BaseTestCase):
    ...
    async def test_get_all_teams_repos_with_api_doc_feature_enabled(self) -> None:
        async with self.db_session.begin() as s:
            ...
            orms1[0].api_documentation_state = FeatureState.ENABLED
            orms2[0].api_documentation_state = FeatureState.ENABLED
    ...
    async def test_get_all_teams_repos_with_inline_code_doc_feature_enabled(self) -> None:
        async with self.db_session.begin() as s:
            ...
            orms1[0].inline_code_documentation_state = FeatureState.ENABLED
            orms2[0].inline_code_documentation_state = FeatureState.ENABLED
    ...
    async def test_get_all_teams_repos_with_arch_doc_feature_enabled(self) -> None:
        async with self.db_session.begin() as s:
            ...
            orms1[0].architecture_documentation_state = FeatureState.ENABLED
            orms2[0].architecture_documentation_state = FeatureState.ENABLED
    ...
    async def test_github_repo_req_create(self) -> None:
        ...
        response = await self.make_request(
            ...
            payload={
                "repo": {
                    ...
                    "display_name": "aaa",
                    "inline_code_documentation_state": FeatureState.ENABLED.value,
                }
            },
            team_id=team.id,
        )
        ...
        assert response_obj.repo.inline_code_documentation_state == FeatureState.ENABLED
        assert response_obj.repo.architecture_documentation_state == FeatureState.DISABLED
        assert response_obj.repo.api_documentation_state == FeatureState.DISABLED
    ...
    async def test_github_repo_req_update(self) -> None:
                ...
                "repos": [
                    {
                        ...
                        "new_values": {
                            "inline_code_documentation_state": FeatureState.PAUSED.value,
                        },
                    }
                    for i in range(2)
                ]
        ...
        assert len(response_obj.repos) == 2
        assert all(
            map(lambda repo: repo.inline_code_documentation_state == FeatureState.PAUSED, response_obj.repos)
    ...
    async def test_github_repo_req_feature_state_query(self) -> None:
        async with self.db_session.begin() as s:
            ...
            orms[1].inline_code_documentation_state = FeatureState.ENABLED
            account = await self.make_account(s, team_id=team.id)
