...
from eave.core.internal.oauth.slack import SlackAuthorizedUser, SlackOAuthResponse, SlackTeam
...
class TestSlackInstallation(BaseTestCase):
    async def asyncSetUp(self) -> None:
        ...
        self.patch(unittest.mock.patch("eave.core.internal.oauth.slack.refresh_access_token_or_exception", return_value={
        ...
        self.patch(unittest.mock.patch(f"{mut}.ResourceMutexOrm", wraps=ResourceMutexOrm))
        self._team = await self.make_team()
    ...
    async def test_refresh_token_with_far_expiry_does_not_refresh(self) -> None:
        ...
        installation_after = await self.reload(installation_before)
        assert installation_after

        async with self.db_session.begin() as db_session:
            await installation_after.refresh_token_or_exception(session=db_session)

        assert self.active_patches["eave.core.internal.oauth.slack.refresh_access_token_or_exception"].call_count == 0
        assert self.active_patches[f"{mut}.ResourceMutexOrm"].acquire.call_count == 0
    ...
    async def test_refresh_token_with_close_expiry_refreshes_token(self) -> None:
        ...
        assert self.active_patches["eave.core.internal.oauth.slack.refresh_access_token_or_exception"].call_count == 1
        assert self.active_patches[f"{mut}.ResourceMutexOrm"].acquire.call_count == 1
        assert self.active_patches[f"{mut}.ResourceMutexOrm"].release.call_count == 1
        assert installation_after.bot_token != self.anystring("bot_token")
        assert installation_after.bot_refresh_token != self.anystring("bot_refresh_token")
        assert installation_after.bot_token_exp
        assert installation_after.bot_token_exp > expiry
    ...
    async def test_refresh_token_without_acquire_doesnt_refresh_token(self) -> None:
        ...
        async with self.db_session.begin() as db_session:
            ...
            acquired = await ResourceMutexOrm.acquire(session=db_session, resource_id=installation_before.id)
            assert acquired
    ...
    async def test_lock_is_released_on_error(self) -> None:
        ...
        self.active_patches["eave.core.internal.oauth.slack.refresh_access_token_or_exception"].side_effect = Exception("test error")
        async with self.db_session.begin() as db_session:
            await installation_after.refresh_token_or_exception(session=db_session)
