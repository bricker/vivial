    ...
    *,
    ...
    if (
        ...
        if new_activity is None:
            return UpdateBookingFailure(failure_reason=UpdateBookingFailureReason.ACTIVITY_SOURCE_NOT_FOUND)
    ...
    async with database.async_session.begin() as db_session:
        ...
        # TODO: account for case where event to update is not present?
        ...
        if input.restaurant_start_time_utc is not None and input.restaurant_start_time_utc is not strawberry.UNSET:
        ...
        # delete event from booking if None was explicitly passed
        if new_activity is None:
