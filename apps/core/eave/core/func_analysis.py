...
async def main():
    for f in funcs:
        ...
        #     lan="en",
        #     n=5, #max n-gram size
        #     dedupLim=0.5, #higher percent allows more word dups across kws
        #     top=25,
        #     features=None,
        # )
        # tags = [kw for kw,_ in kw_extractor.extract_keywords(response)]
        # print(tags)
        # print("\n\n")

        # attempt extract code from steps
        steps = filter(lambda x: len(x) != 0, map(lambda y: y.strip(), response.split("\n")))
        for step in steps:
          extract_prompt = formatprompt(
            f"""
            Give the line numbers that indicate the start and end lines in the code that apply to the following description.
            ```
            {f}
            ```
            Respond with the 2 line numbers separated by a comma.
            """,
          )
                  
          code_spot = await openai_client.chat.completions.create(
            stream=True,
            model="gpt-4-1106-preview",
            messages=[
                ChatCompletionSystemMessageParam({
                    "role": "system",
                    "content": "Only respond with two numbers, separated by a comma. For example: 2,6",
                }),
                ChatCompletionUserMessageParam({
                    "role": "user",
          ...
          code_spot = "".join(filter(None, [chunk.choices[0].delta.content async for chunk in code_spot]))
          print(f"action: {step}\nat lines: {code_spot}\n")
