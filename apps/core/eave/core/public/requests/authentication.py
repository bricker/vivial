...
async def refresh_access_token(input: eave_ops.RefreshAccessToken.RequestBody, request: fastapi.Request) -> eave_ops.RefreshAccessToken.ResponseBody:
    ...
    async with eave_db.get_async_session() as db_session:
        try:
            ...
            logger.error("auth token request invalid. The tokens are invalid or the account has been deactivated.", extra=request_util.log_context(request=request))
...
def make_token_pair(account: eave_orm.AccountOrm, eave_origin: eave_origins.EaveOrigin) -> Tuple[eave_jwt.JWT, eave_jwt.JWT, eave_orm.AuthTokenOrm]:
    sub = str(account.id)
