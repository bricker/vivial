...
class CredsAuthMiddleware(EaveASGIMiddleware):
    async def process_request(
        ...
        try:
            creds: ClientCredentialsOrm
            match self.config.creds_auth_method:
                case CredentialsAuthMethod.headers:
                    creds = await self._get_creds_from_headers(scope)
                case CredentialsAuthMethod.query_params:
                    creds = await self._get_creds_from_qp(request=request, scope=scope)
                case _:
                    raise Exception("Unexpected client credential auth method", self.config.creds_auth_method)
    ...
    async def _get_creds_from_qp(self, request: Request, scope: asgiref.typing.HTTPScope) -> ClientCredentialsOrm:
    ...
    async def _get_creds_from_headers(self, scope: asgiref.typing.HTTPScope) -> ClientCredentialsOrm:
