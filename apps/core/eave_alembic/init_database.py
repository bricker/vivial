...
async def init_database() -> None:
    ...
    async with await eave_db.get_session() as session:
        team = eave_orm.TeamOrm(
            ...
            document_platform=eave_models.DocumentPlatform.confluence
        )

        session.add(team)
        await session.commit()

    print(f"Team created ({team.id}). You'll want to create a ConfluenceDestination for the team. You can do it from a python REPL")
