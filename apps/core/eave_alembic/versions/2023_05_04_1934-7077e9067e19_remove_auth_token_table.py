...
def upgrade() -> None:
    ...
    op.drop_index(
        "ix_auth_tokens_access_token_hashed", table_name="auth_tokens"
    )
    op.drop_index(
        "ix_auth_tokens_refresh_token_hashed", table_name="auth_tokens"
    )
...
def downgrade() -> None:
    ...
    op.create_table(
        ...
        sa.Column(
            "account_id", sa.UUID(), autoincrement=False, nullable=False
        ),
        sa.Column(
            "id",
            sa.UUID(),
        ...
        sa.ForeignKeyConstraint(
            ["team_id"], ["teams.id"], name="auth_tokens_team_id_fkey"
        ),
        sa.PrimaryKeyConstraint(
            "team_id", "account_id", "id", name="auth_tokens_pkey"
        ),
    )
    op.create_index(
        "token_pair",
